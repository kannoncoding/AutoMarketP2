...
namespace AutoMarket.Servidor.Datos
{
    public sealed class VentaDatos
    {
        ...
        public void Actualizar(Venta venta)
        {
            ...
            if (!ExisteId(venta.IdVenta))
            {
                throw new InvalidOperationException("No existe una venta registrada con el id indicado.");
            }
    ...
    IdVenta = @IdVenta;";
            ...
            using (SqlCommand comando = new SqlCommand(consultaSql, conexion))
            {
                ...
                try
                {
                    ...
                    if (filasAfectadas == 0)
                    {
                        throw new InvalidOperationException("No fue posible actualizar la venta.");
                    }
                }
                catch (SqlException ex)
                {
                    throw new InvalidOperationException("Ocurrió un error al actualizar la venta en la base de datos.", ex);
                }
            }
        }
...
WHERE IdVenta = @IdVenta;";
            ...
            using (SqlCommand comando = new SqlCommand(consultaSql, conexion))
            {
                ...
                try
                {
                    ...
                    if (filasAfectadas == 0)
                    {
                        throw new InvalidOperationException("No fue posible eliminar la venta.");
                    }
                }
                catch (SqlException ex)
                {
                    throw new InvalidOperationException("Ocurrió un error al eliminar la venta en la base de datos.", ex);
                }
            }
        }
